...
class ItemsRequestHandler extends RequestHandler implements HandlesPostRequest, HandlesDeleteRequest
{
	...
	{
		if ($id === '-') {
			return $this->proceedDeleteByQueryRequest($request);
		}
	 ...
	 */
	private function proceedDeleteByQueryRequest(ApiRequest $request)
	{
		...
		$this->environmentValidation->validate([
			'query' => $query,
		]);

		dispatch(new DeleteByQuery($query));
